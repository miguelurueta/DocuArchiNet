...
test("registra recursos DOC-12 sin bloques de servidor en la cabecera Web Forms", () => {
    ...
    assert.match(codeBehind, /confirmation-dialog\.css\?v=20260816-doc13/);
    assert.match(codeBehind, /ConfirmationDialog\.js\?v=20260817-doc13error3/);
    assert.match(codeBehind, /workflow-transition-confirmation-integration\.js\?v=20260817-doc13lock1/);
    assert.match(codeBehind, /workflow-transition-page-presentation\.js\?v=20260817-doc13fix2/);
});
...
test("la selección publica el contexto normalizado permitido para la confirmación", () => {
    const ui = loadUi();
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(detail.contexto, "tramite"), false);
    assert.equal(Object.prototype.hasOwnProperty.call(detail.contexto, "actividadOrigen"), false);
});
