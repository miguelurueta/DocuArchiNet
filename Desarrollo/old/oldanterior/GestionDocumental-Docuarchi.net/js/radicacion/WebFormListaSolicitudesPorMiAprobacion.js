$(document).ready(function () {
    $.fn.inicio = function () {
        $('#data_grid_listado_solicitudes tr[id]').click(function () {
            ...
            $('#hdnEmailID').val(fer);
            $('#Hidden_id_solicitud').val($(this).attr("id_solicitud"));
        ...
        //GetLista_documentos_compartidos_mi_aprobacion('TextBox_busqueda');
        active_enter_buton();
        auto_zise_popup_lista_solicitudes("1", "1");
        auto_zise_popup_visor_externo();
        auto_zise_popup_desicion_solicitud();
        ...
        $('#Contenedorgrid_listado_solicitud').contextMenu('context-menu-1', {
            ...
            'Marcar como no visto': {
                click: function (element) {
                    //Button_activa_visto
                    if (document.getElementById("hdnEmailID").value !== "-1" && document.getElementById("hdnEmailID").value !== "0") {
...
function GetLista_documentos_compartidos_mi_aprobacion(name_texbox) {
    ...
    $("#" + name_texbox)
        ...
        .autocomplete({
            source: function (request, response) {
                ...
                $.ajax({
                    url: "../webservice/WebServiceRadicacion.asmx/GetLista_documentos_compartidos_mi_aprobacion",
...
function preven_event_restor_search(event, e) {
    try {
        document.getElementById("Button_restore_lista_service").click();
        event.preventDefault();
    }
    catch (err) {
...
function prevent(event, element) {
    try {
        ...
        var id_solicit = $(element).attr("id_solicitud");
        var tip_event = $(element).attr("tip_event");
        if (tip_event == "ver_doc_sol") {
            $('#hdnEmailID').val(fer);
            $('#Hidden_id_solicitud').val(id_solicit);
           
            document.getElementById("Button_ver_documento_solicitud").click();
        }
        if (tip_event == "desc_doc_resp") {
            $('#hdnEmailID').val(fer);
            $('#Hidden_id_solicitud').val(id_solicit);
            document.getElementById("Button_ver_documento_respuesta_solicitud").click();
        }
...
function pre_actualiza_web_service_estado_visto(id_solicitud, nombre_grid, nombre_campo, color_estado,estado) {
    try {
        ...
        var update = "Update ra_cd_usuarios_solicitudes_aprobacion set ESTADO_VISTO_SOLICITANTE=" + estado + " where ID_CD_USUARIOS_SOLICITUDES_APROBACION=" + id_solicitud;
...
function auto_zise_popup_desicion_solicitud() {
    try {
        ...
        $('#Panel_desicion_solicitud').css("height", (heig_porcent) + "px"); //Asigna altura al panel contenedor del modal
        $('#modal_content_desicion_solicitud').css("height", (heig_porcent - 10) + "px"); // Asigna altura del contenedor bootstraf
        //Asgina el valor del contenido central del modal  contenedor bootstraf  menos la suma del footer y la cabecera
        $('#contenido_procesa_usu_rel_solicitud').css("height", (document.getElementById("modal_content_desicion_solicitud").clientHeight - (document.getElementById("divcabecer_desicion_documento").clientHeight + document.getElementById("modal-footer_desicion_solicitud").clientHeight)) + "px");
        ...
        alert(err.message + " funcion auto_zise_popup_desicion_solicitud " + err.message);
    }
}
