...
Public Class PrinterFileDa
    ...
    Const PRINT_FILE As String = Nothing
    Const EXTENSION_FILE As String = "extension_file"
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Dim Sidruta As String = context.Request("sid2")
        Dim extension As String = context.Request("hideextension")
        fil = Sidruta
        Dim app As HttpApplicationState = HttpContext.Current.Application
        app(sid + EXTENSION_FILE) = extension
        If (String.IsNullOrEmpty(pid) = False) Then
            ...
            Dim extension_file As String = context.Request("extension_file")
            ...
            ClientPrinterSettings(sid, _
                                ...
                                printerCommands, Sidruta, extension)
        Else

            'generate client print job    
            ClientPrint(sid, context, Sidruta)
    ...
    Private Sub ClientPrinterSettings(sid As String, _
                                        ...
                                        printerCommands As String, sidrutas As String, extension As String)

        Try

            Dim app As HttpApplicationState = HttpContext.Current.Application
            app(sid + NET_PRINTER_HOST) = sidrutas
    ...
    Private Sub ClientPrint(sid As String, ByVal context As HttpContext, ByVal sid_ruta As String)
        If (WebClientPrint.ProcessPrintJob(System.Web.HttpContext.Current.Request.Url.Query)) Then
            ...
            Dim spli_general() As String = app(sid + NET_PRINTER_HOST).ToString().Split("|")
            Dim fileextension As String = spli_general(1)
            Dim spli() As String = spli_general(0).ToString().Split(",")
            If Not spli Is Nothing Then
                If spli.Length > 1 Then

                    For i As Integer = 0 To spli.Length - 1
                        If i = 0 Then
                            Dim fileName As String = Guid.NewGuid().ToString("N") + System.IO.Path.GetExtension(spli(i))
                            cpj.PrintFileGroup.Add(New PrintFile(spli(i), fileName + fileextension))
                        Else
                            Dim fileName As String = Guid.NewGuid().ToString("N") + System.IO.Path.GetExtension(spli(i))
                            cpj.PrintFileGroup.Add(New PrintFile(spli(i), fileName + fileextension))
                        End If
                    ...
                    Next
                Else
                    txtFilePath = spli_general(0)
                    Dim fileName As String = Guid.NewGuid().ToString("N") + System.IO.Path.GetExtension(txtFilePath)
                    Dim file As New PrintFile(txtFilePath, fileName & fileextension)
                    cpj.PrintFile = file
                End If
            Else
                txtFilePath = app(sid + NET_PRINTER_HOST).ToString()
                Dim fileName As String = Guid.NewGuid().ToString("N") + System.IO.Path.GetExtension(txtFilePath)
                Dim file As New PrintFile(txtFilePath, fileName & fileextension)
                cpj.PrintFile = file
            End If
