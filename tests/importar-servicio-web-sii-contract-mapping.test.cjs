...
const provider = fs.readFileSync(path.join(root, "Infrastructure/Workflow/ImportarServicioWeb/Sii/SiiExternalImportProviderClient.vb"), "utf8");
...
test("fixtures SII saneados expresan la clave estable", () => {
  ...
  assert.equal(raw.inscripciones[0].imagenes[0].tipoanexo, "505");
...
test("mapper normaliza campos SII y prepara comando documental", () => {
  ...
  assert.match(mapper, /If IsInscriptionAttachment\(image\) Then sealImages\.Add\(image\)/);
  assert.match(mapper, /Value\(image, "tipoanexo"\), "505"/);
  assert.match(mapper, /Value\(sealImage, "idanexo"\)/);
  ...
  assert.match(provider, /sealImages\.Count = 0 Then Throw New InvalidOperationException\("SII_INSCRIPTION_SEAL_MISSING"\)/);
  assert.match(provider, /sealImages\.Count > 1 Then Throw New InvalidOperationException\("SII_INSCRIPTION_SEAL_AMBIGUOUS"\)/);
  assert.match(provider, /String\.Equals\(Value\(sealImage, "idanexo"\), attachmentId, StringComparison\.Ordinal\)/);
  assert.match(service, /SII_INSCRIPTION_SEAL_MISSING/);
  assert.match(service, /SII_INSCRIPTION_SEAL_AMBIGUOUS/);
});
