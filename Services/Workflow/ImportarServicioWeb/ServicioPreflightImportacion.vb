...
Public NotInheritable Class ServicioPreflightImportacion
    ...
    Public Sub New(ByVal validador As ValidadorContextoImportacion)
    ...
    Public Function Preflight(ByVal contexto As ContextoImportacionServicio,
        ...
        For Each item In request.Items
            ...
            response.Commands.Add(New DocumentCommandDto With {.ClientItemId = item.ClientItemId.Trim(), .ExternalKey = item.ExternalKey.Trim(), .DocumentTypeId = item.DocumentTypeId, .FileName = item.FileName, .ContentType = item.ContentType})
        Next
