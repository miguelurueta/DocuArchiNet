...
SCRUMCORE-241: MODULO-REUSABLE-DIGITALIZACIONDOCUMENTAL- WORKBENCH-GRAFICO-CAPTURA-03-FE
...
1. El workbench queda implementado dentro de `DigitalizacionDocumentalModal` para mantener el punto publico existente del modulo y evitar un nuevo contrato de montaje.
2. El estado documental se consume desde `useDigitalizacionDocumentalState`; el estado de scanner, paginas y PDF se consume desde `useDigitalizacionScanner`.
3. `scannerClient` se expone como prop opcional para inyeccion de runtime real o fake de pruebas, con fallback controlado cuando no hay scanner disponible.
4. La UI no crea `object URLs`; renderiza `thumbnailUrl` cuando el adapter lo entrega y mantiene preview textual/estructural con dimensiones estables.
5. El estado local del componente se limita a `selectedPageId`, porque la seleccion de UI no es source-of-truth documental.
6. El boton primario del modal conserva el contrato de FE-01/FE-02 y se habilita cuando existe PDF generado o cuando el estado documental permite submit.

## Risks / Trade-offs

- El guardado/upload real queda fuera de esta fase; la UI deja listo el punto de confirmacion y el PDF generado para la fase de persistencia.
- Algunos estados futuros (`uploading`, `saving`, `resolvingMetadata`) quedan modelados en documentacion pero no se activan hasta integrar backend/metadata.
- Ant Design en pruebas emite warnings de `getComputedStyle`/`act` por efectos asincronos del modal; la suite afectada pasa completa.

## Migration Plan

1. Consumir `scannerClient` real desde el contenedor que abra `DigitalizacionDocumentalModal`.
2. Conectar el `primaryAction` con servicio backend cuando exista contrato de upload/persistencia.
3. Reemplazar placeholder de metadata por selectores reales de TRD/tipologia/lista de chequeo.
4. Mantener pruebas del workbench como regresion para scanner fake y estados contractuales.

## Open Questions

- Definir contrato final de persistencia: guardar documento nuevo vs adjuntar a documento existente.
- Definir si metadata obligatoria se resuelve antes o despues de generar PDF.
- Definir ownership de thumbnails con imagen real si el adapter futuro genera blobs temporales.
