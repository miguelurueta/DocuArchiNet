...
- [ ] 1.1 Crear carpeta `src/app/Components/UI/AppVisorPdf/` con export p\u00fablico (`index.ts`)
- [ ] 1.2 Definir tipos/contrato inicial (`AppVisorPdfProps`) para `source` (URL/Blob/ArrayBuffer) y callbacks `onLoadStateChange`/`onError`
- [ ] 1.3 Crear componente skeleton `AppVisorPdf.tsx` con estados expl\u00edcitos: `idle/loading/ready/error`

## 2. Rendering & UX baseline

- [ ] 2.1 Implementar contenedor de render estable (layout fijo) para evitar flicker en re-render
- [ ] 2.2 Implementar UI b\u00e1sica de loading/error (mensajes + retry opcional) seg\u00fan contrato
- [ ] 2.3 Agregar configuraci\u00f3n determinista: `initialPage`, `initialZoom` (o equivalente) y asegurar que no cambien sin props
