...
El sistema SHALL aplicar validaciones específicas (tipo/pattern) y atributos de accesibilidad apropiados, además de exponer eventos `onChange`, `onBlur` y `onFocus`. Los labels de campos `SELECCION` y `AUTOCOMPLETE` con `campo_tip = 1` SHALL mostrarse con efecto de letra capital.
