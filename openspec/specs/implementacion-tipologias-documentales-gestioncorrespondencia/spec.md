...
Define the frontend contract for Gestion Respuesta attachment typologies using workflow context, preserving the existing AppUploadDocumental flow while loading document typologies from backend with `Contexto=WORKFLOW`, `IdTareaWf`, and `IdRutaWf`.
