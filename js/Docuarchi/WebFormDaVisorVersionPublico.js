...
$(window).on("load", function () {
    ...
    load_page_visor_publico();
});
let VISOR_PUBLIC_FRAME = "";
const load_page_visor_publico = async () => {
    let result = "";
    VISOR_PUBLIC_FRAME = await load_url_page_visor_publico();
    auto_zise_visor_externo();
}
const load_url_page_visor_publico = async () => {
    let myPromise = new Promise(function (resolve) {
        const querystring = window.location.search;
        if (querystring !== "") {
            let params = new URLSearchParams(querystring)
            let empresaOuner = params.get('nameframe');
...
function auto_zise_visor_externo() {
    ...
    if (VISOR_PUBLIC_FRAME !== "") {
         if (window.parent.document.getElementById(VISOR_PUBLIC_FRAME)) {
           espacio_iframe = window.parent.document.getElementById(VISOR_PUBLIC_FRAME).clientHeight;
         }
    }
