...
function activa_menu_general_diference_(event, e, event_name) {
    try {
        if (event_name !== "") {
            ...
            if (event_name == "R-EPA") {
                document.getElementById("Button_devolver_envio").click();
            }
            if (event_name == "R-DDR") {
...
function resize_opcion_descarga_respuesta() {
    try {
        ...
        $("#Contenido_opcion_descarga_respuesta").css("height", heig_ + 50 + "px");
...
function actuo_zise_popup_compartir_correo_electronico() {
    try {
        ...
        $('#ifimpre').css("height", (document.getElementById("contenido_procesa_notifica_gestion").clientHeight - 5) + "px");
    }
    catch (ex) {
