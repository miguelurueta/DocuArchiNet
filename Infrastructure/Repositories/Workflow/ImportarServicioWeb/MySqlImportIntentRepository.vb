            ...
            Using transaction = _transactions.BeginTransaction(connection)
                Try
                    ...
                    For Each item In intent.Resultados
                        _executor.ExecuteNonQuery(connection, transaction, "INSERT INTO workflow_import_intent_item (intent_id,client_item_id,provider_id,external_key,target_task_id,document_type_id,document_type_name,file_name,content_type,status) VALUES (@intentId,@clientId,@providerId,@externalKey,@taskId,@documentTypeId,@documentTypeName,@fileName,@contentType,@status)", New List(Of IDataParameter) From {P("@intentId", intent.Id), P("@clientId", item.ClientItemId), P("@providerId", item.IdentidadExterna.ProviderId), P("@externalKey", item.IdentidadExterna.ExternalKey), P("@taskId", item.IdTareaDestino), P("@documentTypeId", item.IdTipoDocumental), P("@documentTypeName", item.NombreTipoDocumental), P("@fileName", item.NombreArchivo), P("@contentType", item.TipoContenido), P("@status", item.Fase.ToString())})
                    Next
                    transaction.Commit()
