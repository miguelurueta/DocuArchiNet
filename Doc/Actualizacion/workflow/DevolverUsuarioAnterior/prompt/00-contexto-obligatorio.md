...
- Ejecutar una etapa únicamente cuando la solicitud incluya de forma expresa uno de los prompts `01` a `05`.
...
La operación devuelve una tarea activa al usuario Workflow histórico inmediatamente anterior de esa misma tarea. El navegador envía solo `IdTarea` y `TokenVersion`; el servidor resuelve el usuario y actividad desde el historial autorizado. `TokenVersion` es opaco y emitido por servidor: vincula versión de tarea e identificador del registro histórico confirmado, sin aceptar ese identificador como un parámetro independiente del navegador.
...
- La acción se ofrece a todo usuario con contexto Workflow válido; el preview resuelve elegibilidad y presenta bloqueo funcional si no existe un usuario histórico válido. La experiencia moderna es la única ruta de esta operación y no depende de pilotos, listas de usuarios/grupos ni configuración de habilitación.
...
- La operación no consulta, valida, bloquea, crea, actualiza, reasigna ni audita respuestas, radicados o confirmaciones. Los componentes nuevos no pueden referenciar `Classgestionrespuesta`, `Verifica_respuesta_*` ni `Reasigna_respuesta_envia_tarea_usuario`.
- La etapa 01 debe decidir y documentar la política de notificación y de eventos dinámicos de `Terminar_Tarea_Workflow`. El adaptador y los componentes nuevos no pueden construir ni invocar componentes de respuestas. Una construcción transitoria interna del motor legacy es tolerable solo si los parámetros aprobados impiden que ejecute métodos de respuestas; esa condición debe tener prueba focal. Si la notificación requerida dependiera de respuestas, el backend queda bloqueado hasta aprobar un mecanismo alternativo.
...
- La acción moderna se muestra para todo contexto Workflow válido; solo se puede confirmar o ejecutar cuando el preview resuelve un usuario histórico anterior válido.
...
- Un token vencido, historial distinto al confirmado, lock ocupado o solicitud concurrente no provoca una segunda transición. El lock es exclusivo por `IdTarea`; token e historial se revalidan dentro del lock.
...
- La verificación transversal, QA manual y consolidación de evidencia pertenecen a la etapa 04.
