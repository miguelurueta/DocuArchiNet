...
Determinar si la modernización de **Enviar a usuario** está lista para solicitar su activación y preparar una liberación controlada mediante la única configuración existente, `WorkflowCentroTrabajoModernActive`.
...
- No crear una bandera, fuente de configuración, evaluación de autorización, endpoint ni despliegue paralelo para `Enviar a usuario`.
...
- No invocar `After_envio_usuario_workflow`, `Reasigna_respuesta_envia_tarea_usuario` ni otro mecanismo de reasignación de respuesta durante validación, liberación o rollback.
...
3. Están verificadas las evidencias de autorización `CAMBIO_USUARIO`, revalidación de tarea/ruta/flujo/actividad, usuario destino activo y asociado a la ruta, `UTIL_ASIGNA_TAREA=1`, token vencido, concurrencia, respuesta pendiente, auditoría sanitizada y fallback legacy.
4. Ante respuesta pendiente, la operación moderna devuelve un bloqueo funcional y no reasigna la respuesta ni termina la tarea.
5. Con el gate inactivo, `Enviar a usuario` conserva el postback y modal Web Forms legacy; `Continuar flujo` conserva el contrato con `IdConector`.
6. La versión, artefactos de despliegue, configuración objetivo y mecanismo de rollback se pueden identificar sin exponer secretos.
...
4. Verificar, con pruebas autorizadas y no mutantes cuando sea posible, que el gate se evalúa como se espera, que el fallback permanece disponible, que la respuesta pendiente sigue bloqueada sin reasignación y que no hubo regresión de `Continuar flujo`.
5. Ante error, discrepancia, fuga de información, reasignación de respuesta, transición inesperada o ausencia de evidencia, detener la liberación y aplicar el rollback aprobado: gate inactivo, usuarios/grupos vacíos y retorno al postback legacy para nuevos intentos. No revertir transiciones ya confirmadas.
...
- La respuesta pendiente continúa bloqueando la operación sin reasignación.
- Si no hay autorización, el gate termina inactivo y no se modifican usuarios ni grupos.

## DOCUMENTACIÓN TÉCNICA

Crear o actualizar `Doc/Actualizacion/workflow/TerminarUsuario/09-liberacion-activacion-controlada/` con:
