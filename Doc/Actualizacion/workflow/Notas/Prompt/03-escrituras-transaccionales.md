...
- Aplica la política registrada: borrado físico auditado, solo propietario, contenido de texto plano de máximo 16.000 UTF-16, retención heredada de tarea/documento, auditoría sin texto completo e idempotencia de 30 días.
- No ejecutar una escritura si el preflight del esquema MySQL 5.1 no verifica InnoDB para nota y auditoría, `TEXT utf8` para contenido, índices de listado y tabla InnoDB de idempotencia; devuelve `Unavailable` sin cambiar datos. Rechaza caracteres fuera de Unicode BMP.
...
Entregable final: entrega las mutaciones condicionadas, contratos/resultados seguros, pruebas, evidencia de compilación y el preflight verificable de cada esquema objetivo antes de la fase 04.
...
Antes de escribir código, revisa DP-01 a DP-07 del modelo de requerimientos: todos están resueltos. No uses una migración de esquema hasta obtener autorización para aplicarla. Para MySQL 5.1, la migración por esquema convierte `ANOTACION_TAREA` de MyISAM a InnoDB, conserva `Dato_Anotacion TEXT utf8`, agrega índices de operación e histórico, garantiza `wf_log_workflow` InnoDB e incorpora la tabla InnoDB de idempotencia. `utf8mb4` no existe en ese motor; el servicio rechaza caracteres fuera de Unicode BMP.
...
2. Implementa idempotencia: un mismo UUID `clientRequestId` para la misma tarea y autor devuelve durante 30 días la misma respuesta, referencia a nota y resultado; una restricción única de persistencia evita una segunda nota y auditoría.
3. Implementa ActualizarNota y EliminarNota con ETag SHA-256 esperado. La sentencia o unidad de persistencia debe condicionar en forma atómica idNota + idTarea + actor + estado aplicable + valores que componen la versión. No realices una consulta de propietario y luego un UPDATE/DELETE por idNota solamente.
...
6. Audita actor autenticado, tarea validada, nota, ruta, actividad, operación, correlación/clientRequestId, fecha de servidor, resultado y versión resultante. Para contenido registra únicamente SHA-256 y longitud de valor anterior/nuevo; nunca texto completo ni contenido enviado ciegamente por cliente.
7. Valida contenido en servidor: obligatorio, máximo de 16.000 UTF-16, Unicode BMP, comillas y saltos de línea. Rechaza pares sustitutos; se persiste como texto plano y con parámetros sobre `utf8` de MySQL 5.1.
...
- La eliminación es física, atómica y auditada; ningún lector recupera el contenido eliminado.
