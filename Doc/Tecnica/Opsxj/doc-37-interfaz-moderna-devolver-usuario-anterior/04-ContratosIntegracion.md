...
`doc37` agrega al orquestador las etapas `preview`, `execution` y `ui-lock`. El runner acepta una sola etapa por invocación para que cada sesión opere una única tarea seleccionada. Antes de los controles, una sesión sin selección pulsa únicamente el comando oficial de la bandeja para la tarea autorizada y espera la confirmación de Workflow; no manipula campos ocultos, sesión ni servicios internos. `preview` requiere autorización de ambiente y verifica que UI muestre el único destino resuelto por servidor sin cambiar las huellas de estado ni auditoría tomadas después de esa precondición. `execution` y `ui-lock` requieren, además, autorización individual y reservan tareas distintas mediante `DOC37_RESOURCE_CONTRACT`. La etapa de bloqueo retiene la respuesta del endpoint para demostrar que confirmación, cierre, Escape, backdrop y descarga no duplican ni abandonan la única solicitud mutante.
