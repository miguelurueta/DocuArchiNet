# SCRUMCORE-225 - Metadata

- Ticket: `SCRUMCORE-225`
- Autor: `migue` (con soporte de Codex)
- Fecha: `2026-05-22`
- Versión: `1.0`

## Historial de cambios
- 2026-05-22: Creación de artifacts OpenSpec (proposal/design/specs/tasks) y arranque de implementación (selector + preset columnas).
- 2026-05-22: Ajuste de sizing preset para Workbench (Documento `minWidth=60`, Acciones `minWidth=80`) para mejorar visibilidad en panel estrecho.
- 2026-05-22: Documentación ampliada (arquitectura/implementación/pruebas) con trazabilidad y criterios de Go/No-Go.
- 2026-05-22: Commit `a903306` (SCRUMCORE-225: ajuste visual columnas AppTreeTable (Workbench)).
- 2026-05-22: PR `#256` creado: `https://github.com/miguelurueta/DocuArchiCore.react/pull/256`.
- 2026-05-22: Merge a `main` en commit `d425236` (Merge pull request #256).
- 2026-05-22: `opsxj:archive` ejecutado. PR de archive `#257`: `https://github.com/miguelurueta/DocuArchiCore.react/pull/257`.

## Referencias cruzadas
- Arquitectura: `docs/modulos/gestioncorrespondencia/AppTreeTable/AjusteVisualColumnas/SCRUMCORE-225-Arquitectura.md`
- Implementación: `docs/modulos/gestioncorrespondencia/AppTreeTable/AjusteVisualColumnas/SCRUMCORE-225-Implementacion-Detallada.md`
- Pruebas: `docs/modulos/gestioncorrespondencia/AppTreeTable/AjusteVisualColumnas/SCRUMCORE-225-Pruebas.md`
