function doClientPrinttext() {
    try {
        var printerSettings = $("#form1").serialize();
   
        $.post('PrinterTexto.ashx', printerSettings);
        ...
        alert(err.message + " Funcion doClientPrinttext ");
        }

}

function doClientPrinttext_post() {
    try {
    var printerSettings = $("#form1").serialize();

    $.post('PrinterFile.ashx', printerSettings);
    var sessionId = $("#sid").val();
    jsWebClientPrint.print('sid=' + sessionId);
        ...
        alert(err.message + " Funcion doClientPrinttext_post ");
}

}

function doClientPrinttext_post_text() {
    try {
    var printerSettings = $("#form1").serialize();
    $.post('printerf.ashx', printerSettings);
    var sessionId = $("#sid").val();
    jsWebClientPrint.print('sid=' + sessionId);
}
        
    catch (err) {
        alert(err.message + " Funcion doClientPrinttext_post_text ");
}
